...
class Rb_Restaurant_bookings {
	...
	public function on_activate() {
		if ( $this->check_dependencies() ){
			...
			do_action('rb_on_activate_before_flush');
			flush_rewrite_rules();
			$this->maybe_update();
			do_action('rb_plugin_activated');
		} else {
			add_action( 'admin_init', array( $this, 'deactivate' ) );
			wp_die(
	...
	public function on_deactivate() {
		$this->add_roles_and_capabilities();
		do_action('rb_on_deactivate_before_flush');
		flush_rewrite_rules();
		do_action('rb_plugin_deactivated');
	}

	public static function on_uninstall() {
		do_action('rb_plugin_uninstalled');
	}
	...
	protected function init_options() {
		update_option( 'rb_version', self::VERSION );
		add_option( 'rb_db_version', self::DB_VERSION );
	}
	...
	protected function maybe_update() {
		if ( get_option( 'rb_db_version' ) < self::DB_VERSION ) {
			// require_once( self::plugin_dir_path() . 'inc/dep/class-rb-update.php' );
			// new Rb_Update();
			// class Rb_Update()  is missing ??? !!!
		}
	}
...
function rb_restaurant_bookings_init() {
	return Rb_Restaurant_bookings::get_instance();
}
rb_restaurant_bookings_init();
